...
useEffect(()=>{
  ...
  const {getRolesResponsibilities, DeleteRolesResponsibility,DeleteTemplateName ,GetallTemplates} = useContext(SignContext);
  ...
  const getRolesResponsibilitiesData = async () => {
    try {
      ...
      setOriginalRolesResponsibilities(res.data);
  ...
  const handleDeleteRolesResponsibility = async () => {
    ...
    if (selectedForUpdate) {
      ...
      try {
        await DeleteTemplateName(selectedForUpdate);
  ...
  const searchList = (e) => {
    ...
    console.log(originalRolesResponsibilities)
    let filterData = originalRolesResponsibilities.filter(
      (el) =>
        el.TemplateName.toLowerCase().indexOf(inputVal) !== -1
        

    );
    setRolesResponsibilities(filterData);
      ...
      const fetchData = async () => {
        try {
          ...
          if (response.data.length > 0) {
            setIsChecked(response.data[0].Template);
          }
        } catch (error) {
  ...
  const handleCheckboxChange = async (event) => {
    ...
    try {
        const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/pin/updateTemplate/${cleanedUserID}`, {
          Template: checked, // Use the new checked state here for RolesResponsibility
        });
...
const fetchRole = async () => {
  try {
    ...
    const defaultColumn = "TemplateName"; 
    const defaultSortDirection = "asc"; 

    const response = await axios.post(
      `${process.env.REACT_APP_BASE_URL}/template/listTemplates`,
      {
        skip: skip,
        per_page: perPage,
...
useEffect(() => {
  ...
  <div >
    ...
    <div className="col-6">
    <h4 className="mb-0 abc1111">Template</h4>
    </div>
    <div className="col-6">
    <div className="d-flex align-items-center justify-content-end" >
     ...
     <button className="custom_hover btn btn-primary btn-color" type="submit" style={{ display: 'flex' }}>
       ...
       <span className="abc1111">Add Template Name</span>
       </button>
     </Link>
   </div>
      </div>
          ...
          <Row>
            <Col xl={12}>
              ...
              <div class="card-header align-items-center d-flex card-body">
                                    <h4 class="card-title mb-0 flex-grow-1">Template Details</h4>  </div>
